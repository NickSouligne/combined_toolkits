...
def compute_uvalues(
    ...
    """
    Compute Component 3 observed-minus-null differences and u-values
    without generating any figures.

    Returns
    -------
    table_null_delta
        Long-form DataFrame with observed value, null value, and
        observed-minus-null difference for each aggregate statistic.

    table_uval
        One-row DataFrame containing u-values for the six aggregate
        Component 3 statistics.
    """
    est_named = results.get("defs")
    table_null_raw = results.get("table_null")

    if not isinstance(est_named, dict):
        raise ValueError("results['defs'] must be a dict of point estimates.")

    if table_null_raw is None:
        return None, None
    ...
    available_stats = [
        stat for stat in aggregate_stats
        if stat in null_df.columns
    ]
    ...
    observed_df = pd.DataFrame(
        [
            {
                "stat": stat,
                "value_obs": est_named.get(stat, np.nan),
            }
    ...
    table_null_delta = (
        null_df[available_stats]
        .melt(var_name="stat", value_name="value_null")
        .merge(observed_df, on="stat", how="left")
    )

    table_null_delta["obs_minus_null"] = (
        table_null_delta["value_obs"]
        - table_null_delta["value_null"]
    )

    def _uval(stat: str) -> float:
        if stat not in null_df.columns:
            return np.nan
        ...
        observed_value = est_named.get(stat, np.nan)
        ...
        return float(
            np.mean(
                observed_value - null_values > delta_uval
            )
        )
    ...
    return table_null_delta, table_uval
